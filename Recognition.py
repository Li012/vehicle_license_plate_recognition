...
class vsm_model(object):
    ...
    def detect(self, part_card):
        w = abs(part_card.shape[1] - self.SZ) // 2

        # 边缘填充
        part_card = cv2.copyMakeBorder(part_card, 10, 10, w, w, cv2.BORDER_CONSTANT, value=[0, 0, 0])
        # cv2.imshow('part_card', part_card)
    ...
    def detect_chinese(self, part_card):
        w = abs(part_card.shape[1] - self.SZ) // 2
